...
return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="pb-16"> {/* Use className instead of style for consistency */}
        {/* Header */}
        <View className="bg-[#04457E] py-5 px-4 flex-row items-center">
        ...
        <ScrollView>
          ...
          <View className="mx-4 mt-4 bg-[#04457E] p-4 rounded-xl">
            <View className="flex-row items-center">
              <View className="w-14 h-14 bg-[#18FFAA] rounded-full items-center justify-center mr-3">
                <MaterialIcons name="emoji-events" size={30} color="#04457E" />
              </View>
          ...
          <View className="mx-4 mt-4 bg-white p-4 rounded-xl shadow-sm">
            <View className="flex-row items-center">
              <View className="w-14 h-14 bg-[#04457E] rounded-full items-center justify-center mr-3">
                <MaterialCommunityIcons name="robot" size={28} color="#18FFAA" />
              </View>
              <View>
                <Text className="text-[#04457E] text-lg font-bold">AI Assistant</Text>
            ...
            <View key={customer.id} className="mx-4 mt-4 bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              <View className="p-4 flex-row items-center justify-between">
                <View className="flex-row items-center">
                  ...
                  />
                  <View className="ml-3">
                    <Text className="text-lg font-bold text-[#04457E]">{customer.name}</Text>
                    <View className="flex-row items-center">
                      <Text className="text-gray-600">{customer.productType}</Text>
                      <Text className="text-gray-400 mx-1">•</Text>
                      <Text className={`${customer.needsRenewal ? 'text-[#04457E]' : 'text-green-500'}`}>
                        {customer.status}
                      </Text>
                    </View>
                  </View>
                ...
                {/* Status Tag */}
                <View className={`py-1 px-3 rounded-full ${customer.upsellPotential ? 'bg-green-100' : 'bg-[#04457E]'}`}>
                ...
                {customer.aiSuggestion && (
                  <View className="flex-row items-start mt-2">
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-[#04457E] font-medium">{customer.aiSuggestion}</Text>
                    </View>
                  </View>
                )}
              </View>
              ...
              <View className="flex-row p-2 border-t border-gray-100">
                <TouchableOpacity className="flex-1 flex-row items-center justify-center py-2 mr-1">
                  <Ionicons name="logo-whatsapp" size={18} color="#04457E" />
                  <Text className="text-[#04457E] font-medium ml-1">WhatsApp</Text>
                </TouchableOpacity>
                
                <TouchableOpacity className="flex-1 flex-row items-center justify-center py-2 bg-[#18FFAA] rounded-lg mx-1">
                  <Ionicons name="call" size={18} color="#04457E" />
                  <Text className="text-[#04457E] font-medium ml-1">Call</Text>
                </TouchableOpacity>
                
                <TouchableOpacity className="flex-1 flex-row items-center justify-center py-2 ml-1">
                  <Ionicons name="pricetag" size={18} color="#04457E" />
                  <Text className="text-[#04457E] font-medium ml-1">Tag</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))}
